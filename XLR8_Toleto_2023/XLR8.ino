...
void followLine(){
  ...
  if(position>40 && position<60){
    if(speedBaseReal<speedBoost){
      speedBaseReal = speedBaseReal+1;      
    }
  }else{
    speedBaseReal = speedBase;
  }
  
  // Atribuição da velocidade dos motores.
  int speedRight = speedBaseReal + PIDValue;
  int speedLeft  = speedBaseReal - PIDValue;
  ...
  if(speedRight < speedMin) speedRight = -20;
  if(speedLeft  < speedMin) speedLeft  = -20;

  speedRight = map(speedRight, -20, 100, -50, 255);
  speedLeft  = map(speedLeft,  -20, 100, -50, 255);
